...
Update: what I said above is not correct !! it does matter to release the right flag
otherwise you may incorrectly set the pointer to NULL and cause panic
 */
  ...
  for (int i=0; i<MAX_THREADS; i++) {
    if (intersectionVehicles[i] != NULL) {
        if ((intersectionVehicles[i]->origin == origin) && 
         ...
         (lock_do_i_hold(intersectionVehicles[i]->lk))) {
// lock_do_i_hold check if I am set the right pointer to NULL
