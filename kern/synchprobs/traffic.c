...
//kprintf("Thread %d is running check_constraints \n", thread_num);
  int i;
...
//kprintf("thread %d violate the constraint with thread %d\n", thread_num, i);
    kprintf("Vehicle A: ");
...
//kprintf("thread %d pass all check_constraint \n", thread_num);
}
...
//kprintf("thread: %d is running intersection_before_entry\n", (int)thread_num); //LJC
    intersection_before_entry(v.origin, v.destination);
//kprintf("thread: %d finish intersection_before_entry, now enter intersection\n", (int)thread_num);
    ...
    P(mutex);
//kprintf("thread %d am exiting the intersection\n", (int)thread_num);
...
//kprintf("successfully initialize thread: %d\n", i);  // LJC
  }
